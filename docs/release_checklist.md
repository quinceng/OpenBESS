...
- 7d, 30d, 90-day and trailing-12-month historical release cache gates are
  recorded before public annualisation is promoted.
...
- `finance_sensitivities.parquet` and `.csv` expose named low/central/high
  comparison rows.
...
- Dashboard cache includes named finance sensitivity files.
...
- Full OpenBESS Stack Index labelling is used only when the
  `openbess_canonical_1mw_2mwh` primary window is `trailing_12m`,
