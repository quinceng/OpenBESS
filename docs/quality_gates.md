...
Promotion from `OpenBESS Stack Index Preview` to `OpenBESS Stack Index` requires
a cache for `openbess_canonical_1mw_2mwh` whose primary window is
`trailing_12m`, whose `target_window_eligible` manifest field is `true`, and
whose caveat flags do not include `below_trailing_12m_coverage`. A 90-day cache
may satisfy the Release 1 public annualisation floor, but it remains preview
evidence until the trailing-12-month gate passes.
